...
- **결과: 3/3 held-out 문항 모두 JS divergence 개선(-37%~-48%)** — 순환논리 없이(test 문항 실제값은 채점에만 사용) 단일 공유계수가 일반화됨을 확인.
- **의미:** 가상조사가 척도상 중저점 쪽에 쏠리는 편향(학습계수 c≈+0.4~1.0)이 문항특이적이 아니라 파이프라인 전반의 구조적 패턴임을 증명.
- **문서화:** report_compare.html 12절(방법론·결과표·한계) 완료.
- **한계(기록됨):** 앵커 3문항뿐(통계력 제한), scale5 전용(rank/branch/open 미적용), 사후분석 단계이며 수집 파이프라인엔 미통합.
- **남은 선택과제:** ① PDF서 추가 완전분포 확보해 앵커 확대, ② rank/branch용 보정 설계, ③ 실제 수집 파이프라인에 통합 여부 결정.

## ✅ 최종 상태 — 개선 작업 완료, 확정 채택
- **결론:** 개선 파이프라인(A확률추출+B잠재성향+C백스토리+거주지애착프라이어+순위형PL+DK/불성실주입+완화 응답스타일변환·프롬프트)을 최종안으로 확정.
...
| **v5(n25)** | 계수0.15+완화프롬프트(동일코드,재추출) | **3.73** | **3.30** | **3.10** | 70.1 | **63.7** |
→ v2→v3: 계수완화 효과 미미(원인=프롬프트 확인). **v4·v5(둘 다 완화프롬프트, 소표본 재추출 2회): 과잉보정 이전(v3: 3.55·2.73·3.05)보다 명백히 개선되어 3.7~4.0대(Q6)·3.16~3.30(Q16)·3.1~3.41(Q28) 구간에 수렴. Q22 접근성1위·수치(43.6→44.5→45.6)는 세 번 모두 실제와 거의 일치, 가장 안정적.** v4·v5 간 자체 편차(Q6 3.97 vs 3.73, Q29 75.4 vs 63.7)는 n=18·25 소표본 통계노이즈로 판단 — **완화프롬프트가 과잉보정을 해소한다는 방향성 결론은 재확인(2회 반복)됨.**

### ✅ 결론: 개선 파이프라인(확률추출+잠재성향+애착프라이어+완화프롬프트) 확정 채택.
- phase1은 n=60 두 번(v4-p1 48성공, v5-p1 60/60 완벽) 모두 안정적으로 Q6 3.6~3.7대 확인.
- phase2는 토큰한도상 매번 부분성공(n18→25)에 그침 — **더 정밀한 소수점 확정은 무의미**(±0.2~0.3 변동은 소표본 노이즈). 방향성(과잉보정 해소·분산유지)은 충분히 검증됨.
- **다음 단계(선택):** ① 여러 소표본을 모두 합쳐(v4+v5 phase2, n≈43) 재계산해 정밀도 높이기, ② 그대로 이 결론을 report_compare.html 10절에 "해결됨"으로 반영, ③ 정 안심 안되면 phase2만 토큰 완전리셋 직후 n=60 단독 재시도(interval 30s 이상으로 늦춰 완주율↑).
- 막히면(과잉보정 재현시) 계획#4(문항별 선택보정, JS divergence).
